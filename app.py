    ...
    if not conversation.strip():
        ...
        with st.spinner(
            ...
            try:
                ...
                # Reset review state for the new lead
...
if "result" in st.session_state:
    ...
    if st.session_state["followup_reviewed"]:
        ...
        st.info(
            "Review the AI-generated follow-up before "
            "approving it."
        )
...
with st.expander("🧹 Database Cleanup"):
    ...
    if st.button(
        "🗑️ Clear All Leads",
        type="secondary",
        key="clear_all_leads_button",
    ):

        deleted = clear_all_leads()

        # Remove current result from session
        if "result" in st.session_state:
